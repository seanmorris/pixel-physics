  ...
  _createClass(PointActor, [{
    ...
    value: function updateGroundPosition() {
      ...
      if (this["public"].gSpeed) {
        ...
        var direction = Math.sign(this["public"].gSpeed);
    ...
    value: function updateAirPosition() {
      ...
      var speed = this["public"].xSpeed;

      if (this["public"].height > this.maxStep) {
        var foreDistance = this.scanForward(this["public"].xSpeed + 1 * Math.sign(this["public"].xSpeed));

        if (foreDistance !== false) {
          var space = 1 + foreDistance;
      ...
      var airPoint = this.castRay(airSpeed, this["public"].airAngle, function (i, point) {
      ...
      this.willJump = false;
      var blockers = false;
      var upMargin = this["public"].flying ? this["public"].height + this["public"].yMargin : 1;
      ...
      for (; col < Math.abs(offset); col++) {
        ...
        downFirstSolid = this.castRay(maxStep * (1 + col), this.downAngle, offsetPoint, function (i, point) {
          var actors = viewport.actorsAtPoint.apply(viewport, _toConsumableArray(point)).filter(function (a) {
            ...
            return (i <= 1 || _this6["public"].gSpeed) && a.callCollideHandler(_this6);
          }).filter(function (a) {
            return a.solid;
          });
          ...
          if (tileMap.getSolid.apply(tileMap, _toConsumableArray(point).concat([_this6["public"].layer]))) {
            return i;
          }
        });
    ...
    value: function scanForward(speed) {
      ...
      return this.castRay(scanDist, this["public"].falling ? Math.sign(speed) > 0 ? 0 : Math.PI : this.realAngle + (Math.sign(speed) < 0 ? Math.PI : 0), startPoint, function (i, point) {
        ...
        if (tileMap.getSolid.apply(tileMap, _toConsumableArray(point).concat([_this7["public"].layer]))) {
          return i;
        }
      });
    }
    ...
    value: function scanBottomEdge() {
      ...
      return this.castRay(this["public"].width, direction < 0 ? Math.PI : 0, [-direction * (this["public"].width / 2), 0], function (i, point) {
        ...
        if (!actors.length && !tileMap.getSolid(point[0], point[1] + 1, _this8["public"].layer)) {
          return i;
        }
      });
    }
    ...
    value: function scanVerticalEdge() {
      ...
      return this.castRay(this["public"].height + 1, Math.PI / 2, [direction * this["public"].width / 2, -this["public"].height], function (i, point) {
        ...
        if (actors.length || tileMap.getSolid.apply(tileMap, _toConsumableArray(point).concat([_this9["public"].layer]))) {
          return i;
        }
      });
    }
    ...
    get: function get() {
      ...
      var groundAngle = this["public"].groundAngle;
      var trajectory;
...
var Region = /*#__PURE__*/function (_PointActor) {
  ...
  _createClass(Region, [{
    ...
    value: function update() {
      ...
      var frame = this.viewport.args.frameId;
  ...
  _createClass(Spring, [{
    ...
    value: function collideA(other) {
      ...
      this.onTimeout(64, function () {
        ...
        other.args.falling = true;
        other.impulse(_this2.args.power, _this2.args.angle, true);
      });
    }
  }, {
    key: "canStick",
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var replayUrl = '/debug/replay-updated.json';
  var replay = fetch(replayUrl);
  Promise.all([viewportA.tileMap.ready, replay]).then(function (_ref) {
...
var Viewport = /*#__PURE__*/function (_View) {
  ...
  function Viewport(args, parent) {
    ...
    _this.args.bindTo('frameId', function (v) {
      return _this.args.frame.args.value = Number(v);
    });
    ...
    if (_this.replayInputs.length) {
      _this.args.hasRecording = true;
    }

    _this.args.topLine.args.value = ' i cant believe its not canvas. ';
    _this.args.status.args.value = ' click here to exit demo. ';
    return _this;
  }
  ...
  _createClass(Viewport, [{
    ...
    key: "readInput",
    value: function readInput() {// if(!this.controlActor)
      // {
      // 	return;
      // }
    ...
    value: function moveCamera() {
      ...
      var xNext = -this.controlActor.x + this.args.width * 0.5;
    ...
    value: function update() {
      ...
      this.args.frameId++;
      var actorThree = this.actorsById[3];
      ...
      for (var i in this.actors.list) {
        ...
        if (!(_actor instanceof _Region.Region)) {
    ...
    value: function getNearbyColCells(actor) {
      var actorX = actor["public"] ? actor["public"].x : actor.x;
      var actorY = actor["public"] ? actor["public"].y : actor.y;
      var colCellDiv = this.colCellDiv;
      var cellX = Math.floor(actorX / colCellDiv);
      var cellY = Math.floor(actorY / colCellDiv);
      var name = "".concat(cellX, "::").concat(cellY);
      var cache = this.colCellCache[name];
      ...
      this.colCellCache[name] = cache = [this.getColCell({
        x: actorX - space,
        y: actorY - space
      }), this.getColCell({
        x: actorX - space,
