        ...
        if (typeof object[key] === 'function') {
          ...
          var wrappedMethod = function wrappedMethod() {
            ...
            if (new.target) {
              ...
              if (isMethod) {
                ret = object[Unwrapped][key].apply(objRef || object, providedArgs);
              } else {
...
var MainMenu = /*#__PURE__*/function (_Card) {
  _inherits(MainMenu, _Card);
  ...
  _createClass(MainMenu, [{
    ...
    value: function onRendered() {
      ...
      var elements = _toConsumableArray(bounds.querySelectorAll(this.selector));

      this.listen(elements, 'focus', function (event) {
        return _this2.currentItem = event.target;
      });
      this.listen(elements, 'blur', function (event) {
        return event.target.classList.remove('focused');
      });
    ...
    value: function onAttached() {
      var next = this.findNext(this.currentItem, this.tags.bound.node);
...
var Block = /*#__PURE__*/function (_PointActor) {
  ...
  _createClass(Block, [{
    ...
    key: "solid",
    get: function get() {
      return this["public"]["float"] || this["public"].falling;
    }
  }]);
  ...
  _createClass(PointActor, [{
    ...
    value: function onRendered() {
      ...
      this.listen('click', function () {
        ...
        var _iterator2 = _createForOfIteratorHelper(_this2.viewport.tags.currentActor.options),
            _step2;
    ...
    value: function update() {
      ...
      var _iterator3 = _createForOfIteratorHelper(this.behaviors),
          _step3;
    ...
    value: function updateAirPosition() {
      ...
      if (!this.willStick && this.args.xSpeed) {
        var foreDistance = this.scanForward(this["public"].xSpeed, false);
    ...
    value: function resolveIntersection() {
      ...
      if (!this.args.falling || !this["public"].xSpeed && !this["public"].ySpeed) {
        return;
      }
    ...
    value: function doJump(force) {
      ...
      var sensorSpread = this["public"].width;
      var jumpAngle = Math.atan2(forePosition[1] - backPosition[1], sensorSpread + 1);
      this.args.ignore = 6;
      this.args.landed = false;
      this.args.falling = true;
  ...
  _createClass(Input, [{
    ...
    value: function init(buttonId) {
      ...
      var hold = setInterval(function () {
        return controller.replay(frame);
      }, 16);
...
_defineProperty(Input, "useText", 'input x [y = 500]');
});
...
_defineProperty(Pos, "helpText", 'Move the current actor to a position in space.');
});
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var lastTime = 0; // const replayUrl = '/debug/replay-updated.json';
  // const replay = fetch(replayUrl);

  Promise.all([viewportA.tileMap.ready]).then(function (_ref) {
    var _ref2 = _slicedToArray(_ref, 2),
        tileMap = _ref2[0],
        replayResult = _ref2[1];

    viewportA.startLevel();
  }).then(function (replay) {
    viewportA.replayInputs = replay;

    if (replay && replay.length) {// viewportA.args.hasRecording = true;
...
var _BootCard = require("./BootCard");

var _SeanCard = require("./SeanCard");

var _MainMenu = require("../Menu/MainMenu.js");

var _TitleScreenCard = require("./TitleScreenCard");
...
var Series = /*#__PURE__*/function (_View) {
  ...
  function Series() {
    ...
    , new _SeanCard.SeanCard({
      timeout: 5000
    }, parent), new _TitleScreenCard.TitleScreenCard({
      timeout: 50000
    }, parent), new _MainMenu.MainMenu({
      timeout: -1
    }, parent), new _Titlecard.Titlecard({
...
var RtcClient = /*#__PURE__*/function (_Mixin$with) {
  ...
  function RtcClient(rtcConfig) {
    ...
    _this.peerClientChannel.addEventListener('open', function () {
      ...
      var messageEvent = new CustomEvent('open', {
        detail: event.data
      });
    ...
    _this.peerClient.createOffer().then(function (offer) {
      _this.peerClient.setLocalDescription(offer);
    });
...
var RtcServer = /*#__PURE__*/function (_Mixin$with) {
  ...
  function RtcServer(rtcConfig) {
    ...
    _this.answerToken = new Promise(function (accept) {
      _this.peerServer.addEventListener('icecandidate', function (event) {
        if (!event.candidate) {
          return;
        }

        accept(_this.peerServer.localDescription);
      });
    });

    _this.peerServer.addEventListener('iceconnectionstatechange', function () {
      var state = _this.peerServer.iceConnectionState;
      console.log("RTC state: ".concat(state));
    });
    ...
    _this.peerServer.addEventListener('datachannel', function (event) {
      ...
      _this.peerServerChannel.addEventListener('open', function () {
        ...
        console.log('Remote peer client accepted!');
...
var RtcServerTask = /*#__PURE__*/function (_Task) {
  ...
  _createClass(RtcServerTask, [{
    ...
    value: function init() {
      ...
      this.server = new _RtcServer.RtcServer({// iceServers: [
...
var Viewport = /*#__PURE__*/function (_View) {
  ...
  function Viewport(args, parent) {
    ...
    _this.vizi = true;
    _this.args.screenFilter = 'runners';
    _this.sprites = new _Bag.Bag();
    _this.world = null;
    _this.args.titlecard = new _Series.Series({}, _assertThisInitialized(_this));
    ...
    _this.colCellDiv = _this.args.width > _this.args.height ? _this.args.width * 0.5 : _this.args.height * 0.5;
    ...
    _this.replayInputs = [];
    _this.args.backdrop = new _MarbleGarden.MarbleGarden(); // this.replayInputs = JSON.parse(localStorage.getItem('replay')) || [];
    ...
    _this.listen(document, 'keydown', function (event) {
      if (event.key === 'F10') {
        if (!_this.args.subspace) {
          ...
          _this.args.subspace.tasks.bindTo(function (v, k) {
            console.log(k, v);
          });
        }
  ...
  _createClass(Viewport, [{
    ...
    value: function startLevel() {
      ...
      this.args.titlecard.play().then(function (done) {
        _this5.args.paused = false;
        _this5.args.started = true;
        _this5.startTime = Date.now();
        Promise.all(done).then(function () {
          return _this5.args.titlecard = '';
        });
      });
    }
  }, {
