  ...
  _createClass(BrokenMonitor, [{
    ...
    value: function collideB(other) {
      ...
      if (other.solid && this.args.collType === 'collision-bottom' && other.y > this.y) {
        ...
        // this.debindX  && this.debindX();
        this.debindY && this.debindY();
        this.restingOn = other; // this.debindYs = other.args.bindTo('ySpeed', v => this.args.gSpeed = v);
...
var MarbleBlock = /*#__PURE__*/function (_PointActor) {
  ...
  _createClass(MarbleBlock, [{
    ...
    value: function collideA(other, type) {
      _get(_getPrototypeOf(MarbleBlock.prototype), "collideA", this).call(this, other);
      ...
      if (type === 3 || type === 1) {
        ...
        var speed = Math.abs(other.args.gSpeed);
        ...
        var nextPos = this.findNextStep(other.args.gSpeed);
  ...
  _createClass(Monitor, [{
    ...
    value: function collideA(other) {
      ...
      if (this.args["float"] === -1 && this.args.collType === 'collision-bottom') {
        ...
        other.args.ySpeed = 0;
        return true;
      } else if (this.args.collType !== 'collision-bottom' && (!this.args.falling || this.args["float"] === -1) && (Math.abs(other.args.xSpeed) > 64 || Math.abs(other.args.gSpeed) > 64 || other.args.falling && other.args.ySpeed > 1 && other.y < this.y) && this.viewport && !this.gone) {
        // other.args.xSpeed *= -1;
        other.args.ySpeed *= -1;
  ...
  _createClass(PointActor, [{
    ...
    value: function update() {
      ...
      if (!this.isEffect && this.args.falling) {
        ...
        var airPoint = this.castRay(airSpeed, this.args.airAngle, function (i, point) {
          ...
          var actors = (_this2$viewport = _this2.viewport).actorsAtPoint.apply(_this2$viewport, _toConsumableArray(point)).filter(function (x) {
            ...
            return x.callCollideHandler(_this2);
          }).filter(function (x) {
          ...
          if (tileMap.getSolid.apply(tileMap, _toConsumableArray(point).concat([_this2.args.layer]))) {
            return lastPoint;
          }
          ...
          if (!blockers) {
            ...
            if (this.willJump) {
              ...
              blockers = blockers.filter(function (a) {
                return a.callCollideHandler(_this2);
              });
      ...
      if (!this.isEffect && !this.args.falling) {
        ...
        } else {// const sensorSpread = this.args.width / 2;
      ...
      if (!this.isEffect) {
        ...
        if (this.getMapSolidAt(this.x, this.y)) {
          ...
          while (true) {
            _blockers2 = this.getMapSolidAt(testX, testY);

            if (!_blockers2) {
              break;
            }

            if (Array.isArray(_blockers2)) {
              _blockers2 = _blockers2.filter(function (x) {
                return x !== _this2;
              }).filter(function (x) {
                return x.callCollideHandler(_this2);
              });

              if (!_blockers2.length) {
                break;
              }
            }
      ...
      if (!this.args.falling) {
        ...
        var scanDownLeft = this.castRay(this.maxStep + 1, this.downAngle, [half > 1 ? -half : 0, 0], function (i, point) {
          var _this2$viewport3;

          var actors = (_this2$viewport3 = _this2.viewport).actorsAtPoint.apply(_this2$viewport3, _toConsumableArray(point)).filter(function (a) {
            return a !== _this2;
          }).filter(function (a) {
            return a.callCollideHandler(_this2);
          }).filter(function (a) {
        ...
        var scanDownRight = !half ? scanDownLeft : this.castRay(this.maxStep + 1, this.downAngle, [half > 1 ? -1 + half : 0, 0], function (i, point) {
          var _this2$viewport4;

          var actors = (_this2$viewport4 = _this2.viewport).actorsAtPoint.apply(_this2$viewport4, _toConsumableArray(point)).filter(function (x) {
            return x !== _this2;
          }).filter(function (a) {
            return a.callCollideHandler(_this2);
          }).filter(function (x) {
        ...
        }); // this.args.type === 'actor-generic' && console.log(scanDownLeft, scanDownRight);

        if ((scanDownLeft === false || scanDownLeft > 1) && (scanDownRight === false || scanDownRight > 1) && !this.args.ignore) {
          this.args.falling = true;
          this.args.xSpeed = 0; //-1 * Math.sign(this.args.xSpeed);

          this.args.ySpeed = 0;
          this.args.ignore = 8;
        } else {// this.args.falling = false;
        }
      }
    ...
    value: function findNextStep(offset) {
      ...
      var prevUp = 0,
          ...
          prev = 0;
      var half = Math.floor(this.args.width / 2) || 0;
      var face = half * this.args.direction;
      var col = 0;
      ...
      for (; col < Math.abs(offset); col++) {
        ...
        downFirstSolid = this.castRay(maxStep * (1 + col), this.downAngle, offsetPoint, function (i, point) {
          var actors = viewport.actorsAtPoint.apply(viewport, _toConsumableArray(point)).filter(function (x) {
            ...
            return (i <= 1 || _this3.args.gSpeed) && a.callCollideHandler(_this3);
          }).filter(function (x) {
            return x.solid;
          });
          ...
          if (tileMap.getSolid.apply(tileMap, _toConsumableArray(point).concat([_this3.args.layer]))) {
            return i;
          }
        });
        ...
        if (Math.abs(downDiff) > maxStep) {
          ...
          return [false, false, false, true]; // return [(-1+col) * sign, prev, false, true];
        }
        ...
        if (downFirstSolid === 0) {
          ...
          switch (this.args.mode) {
            case MODE_FLOOR:
              _offsetPoint = [columnNumber + face, 0];
              break;

            case MODE_RIGHT:
              _offsetPoint = [0, -columnNumber + -face];
              break;
...
var QuestionBlock = /*#__PURE__*/function (_PointActor) {
  ...
  function QuestionBlock() {
    ...
    _defineProperty(_assertThisInitialized(_this), "maxBounce", 8);
  ...
  _createClass(Spring, [{
    ...
    value: function collideA(other) {
      ...
      if (this.args.collType === 'collision-top') {
        other.impulse(this.args.power, -Math.PI / 2, true); // other.args.ySpeed  = -this.args.power || -15;
...
var CharacterString = /*#__PURE__*/function (_View) {
  ...
  function CharacterString() {
    ...
    _defineProperty(_assertThisInitialized(_this), "template", "<div class = \"hud-character-string [[hide]] [[alert]]\" cv-each = \"chars:char:c\" style = \"--scale:[[scale]];\"><span\n\t\t\t\tclass = \"hud-character\"\n\t\t\t\tdata-type  = \"[[char.type]]\"\n\t\t\t\tdata-value = \"[[char.pos]]\"\n\t\t\t\tdata-index = \"[[c]]\"\n\t\t\t\tstyle      = \"--value:[[char.pos]];--index:[[c]];--length:[[chars.length]];\"\n\t\t\t>[[char.original]]</span></div>");
    ...
    _this.args.bindTo('value', function (v) {
      if (Math.abs(v) > _this.args.high) {
        _this.args.alert = 'red';
      } else if (Math.abs(v) > _this.args.med) {
        _this.args.alert = 'orange';
      } else if (Math.abs(v) > _this.args.low) {
        _this.args.alert = 'yellow';
      } else {
        _this.args.alert = '';
      }
...
var Viewport = /*#__PURE__*/function (_View) {
  ...
  function Viewport(args, parent) {
    ...
    _this.args.width = 32 * 12;
    _this.args.height = 32 * 7;
    _this.args.scale = 2;
    _this.args.x = 0;
    _this.args.y = 0;
    _this.args.layers = [];
  ...
  _createClass(Viewport, [{
    ...
    value: function fullscreen() {
      ...
      this.tags.viewport.requestFullscreen().then(function (res) {
        requestAnimationFrame(function () {
          ...
          var vScale = window.innerWidth / _this2.args.width; // this.args.scale = hScale > vScale ? hScale : vScale;
        });
      });
    }
  }, {
        ...
        _this3.onTimeout(750, function () {
          ...
          _this3.onTimeout(500, function () {
            ...
            _this3.onTimeout(1500, function () {
              ...
              _this3.args.status.args.value = ' Click here for keyboard control. ';
    ...
    value: function update() {
      ...
      if (!this.args.actors.length) {
        ...
        for (var i in objDefs) {
          if (this.args.actors.length) {// continue;
          }
          ...
          for (var _i in objDef.properties) {
            var property = objDef.properties[_i];
            objArgs[property.name] = property.value;
          }

          var obj = new objClass(Object.assign({}, objArgs));
          this.actors.add(obj);
        }
      ...
      if (this.gamepad) {
        ...
        for (var _i2 = 0; _i2 < gamepads.length; _i2++) {
          var gamepad = gamepads.item(_i2);
      ...
      if (_Keyboard.Keyboard.get().getKey('PageUp')) {
        this.args.actors[0].args.layer = 1;
      } else if (_Keyboard.Keyboard.get().getKey('PageDown')) {
      ...
      for (var _i3 in this.args.actors) {
        var actor = this.args.actors[_i3];
        this.setColCell(actor);
        actor.updateStart();
      }

      for (var _i4 in this.args.actors) {
        var _actor = this.args.actors[_i4];
      ...
      for (var _i5 in this.args.actors) {
        var _actor2 = this.args.actors[_i5];
      ...
      for (var _i6 = 0; _i6 < layerCount; _i6++) {
        if (!this.args.layers[_i6]) {
          this.args.layers[_i6] = new _Layer.Layer({
            layerId: _i6
          });
          this.args.layers[_i6].args.height = this.args.height;
          this.args.layers[_i6].args.width = this.args.width;
        }

        this.args.layers[_i6].x = this.args.x;
        this.args.layers[_i6].y = this.args.y;

        this.args.layers[_i6].update(this.tileMap);
      }
      ...
      for (var _i7 in this.args.actors) {
        var _actor3 = this.args.actors[_i7];
    ...
    value: function getNearbyColCells(actor) {
      ...
      return this.colCellCache[name] = [this.getColCell({
        x: actor.x - colCellDiv,
        y: actor.y - colCellDiv
      }), this.getColCell({
        x: actor.x - colCellDiv,
        y: actor.y + 0
      }), this.getColCell({
