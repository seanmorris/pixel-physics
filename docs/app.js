...
var PointActor = /*#__PURE__*/function (_View) {
  ...
  _createClass(PointActor, [{
    ...
    value: function updateAirPosition() {
      ...
      var speed = this.args.xSpeed;

      if (this.args.height > this.maxStep) {
        var foreDistance = this.scanForward(this.args.xSpeed + 1 * Math.sign(this.args.xSpeed));
  ...
  _createClass(StarPost, [{
    ...
    value: function collideA(other) {
      ...
      if (!this.args.active) {
        ...
        if (Math.abs(throwSpeed) > 10) {
          throwSpeed = 10 * Math.sign(throwSpeed);
        }
  ...
  _createClass(Viewport, [{
    ...
    value: function updateBackground() {
      ...
      if (this.controlActor && this.tags.blur) {
        var xBlur = Number((this.controlActor.x - this.xPrev) * 100 / 500).toFixed(4);
        var yBlur = Number((this.controlActor.y - this.yPrev) * 100 / 500).toFixed(4);
        var blurAngle = Number(this.controlActor.realAngle + Math.PI).toFixed(4);
        var maxBlur = 16;
        xBlur = xBlur < maxBlur ? xBlur : maxBlur;
        yBlur = yBlur < maxBlur ? yBlur : maxBlur;
        var blur = Math.sqrt(Math.pow(xBlur, 2) + Math.pow(yBlur, 2)) / 4;
        this.tags.blurAngle.setAttribute('style', "transform:rotate(".concat(blurAngle, "rad)"));
        this.tags.blurAngleCancel.setAttribute('style', "transform:rotate(".concat(-blurAngle, "rad)"));
    ...
    value: function update() {
      ...
      var nearbyCells = this.controlActor ? this.getNearbyColCells(this.controlActor) : {};
